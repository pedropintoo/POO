...
public class P002021 {
	...
	private static void alinea2(PrintStream out) throws FileNotFoundException {
        ...
        Scanner input = new Scanner(new FileReader("src/uafest.txt"));
        ...
        while(input.hasNextLine()){
            ...
            switch (line[0]){
                case "1" -> {
                    Gastronomia ps1 = new Gastronomia(Integer.parseInt(line[1]),line[2]);
                    fest.add(ps1);
                }
                case "2"-> {
                    List<String> locaisList = new ArrayList<>(Arrays.asList(line).subList(3, line.length));
                    String[] locais = new String[locaisList.size()];
                    for(int i = 0; i < locaisList.size(); i++){
                        locais[i] = locaisList.get(i);
                    }
                    PasseioBicicleta ps2 = new PasseioBicicleta(Integer.parseInt(line[1]),line[2],locais);
                    fest.add(ps2);
                }
            }
