	...
	private static void alinea2(PrintStream out) throws FileNotFoundException {
		...
		*/
        ...
        while(input.hasNextLine()){
            ...
            client = campingService.getClient(Integer.parseInt(content[0]));

            startDate = LocalDate.parse(content[1]);
            duration = Integer.parseInt(content[2]);

            spaceSet = campingService.findAvailableCampingSpaces(SpaceType.valueOf(content[3]),startDate, duration,new int[] {Integer.parseInt(content[4].split("x")[0]), Integer.parseInt(content[4].split("x")[1])} );
            if(spaceSet.size() > 0){
                campingService.bookCampingSpace(client, spaceSet.get(0), startDate,duration);
            }

        }

        campingService.listBookings(SpaceType.CAR).forEach(out::println);
