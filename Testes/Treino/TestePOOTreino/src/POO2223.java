...
public class POO2223 {
	...
	private static void alinea1(PrintStream out) {
		...
		campingService.registerClient(501234567, "Arlindo Marques", ClientType.MEMBER); //membro
		campingService.registerClient(501987654, "Júlio Pomar", ClientType.MEMBER); //membro
	...
	private static void alinea2(PrintStream out) throws FileNotFoundException {
        ...
        Scanner input = new Scanner(new FileReader("src/bookings.txt"));
        ...
        while (input.hasNextLine()) {
            line = input.nextLine();
            content = line.split(";");
            Client client = campingService.getClient(Integer.parseInt(content[0]));
            LocalDate startDate = LocalDate.parse(content[1]);
            int duration = Integer.parseInt(content[2]);
            SpaceType spaceType = SpaceType.valueOf(content[3]);
            String[] contentDim = content[4].split("x");
            int[] minDim = {Integer.parseInt(contentDim[0]), Integer.parseInt(contentDim[1])};

            List<CampingSpace> campingSpaces = campingService.findAvailableCampingSpaces(spaceType, startDate, duration, minDim);
            if(client != null && campingSpaces != null && campingSpaces.size() > 0){
                CampingSpace space = campingSpaces.get(0);
                campingService.bookCampingSpace(client,space,startDate,duration);
            }
        }
		...
		List<CampSpace.CampingSpace> available = campingService.findAvailableCampingSpaces(SpaceType.CAR, LocalDate.of(2023, 6, 1), 60, new int[]{300, 600});
		if (available==null || available.isEmpty()) {
			out.println("** No options available for the type, dimensions, and dates requested.\n");
		} else {
			CampSpace.CampingSpace u = available.get(0);
